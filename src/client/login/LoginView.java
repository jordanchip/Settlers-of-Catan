...
public class LoginView extends OverlayView implements ILoginView {
	...
	private JComponent initLeftComponents() {
		...
		JLabel lblTitle = new JLabel(
				"<html><body>Settlers<br>of Catan</body></html>");
	...
	private Border createBufferBorder() {
		...
		Border innerBuffer = BorderFactory.createEmptyBorder(BUFFER_SPACE,
				BUFFER_SPACE, BUFFER_SPACE, BUFFER_SPACE);
		Border outerBuffer = BorderFactory.createEmptyBorder(BUFFER_SPACE,
				BUFFER_SPACE, BUFFER_SPACE, BUFFER_SPACE);
		Border etching = BorderFactory.createEtchedBorder();

		Border outerCompound = BorderFactory.createCompoundBorder(outerBuffer,
				etching);
		Border wholeCompound = BorderFactory.createCompoundBorder(
				outerCompound, innerBuffer);
	...
	private class RegisterPanel extends JPanel {
		...
		private void initTooltips() {
			txtUsername
					.setToolTipText("The username must be between three and seven "
			...
			txtPasswordAgain
					.setToolTipText("Make sure the two passwords match!");
		}
		...
		private void initEventListeners() {
			...
			TextFieldValidator usernameValidator = new TextFieldValidator(
					txtUsername) {
				@Override
			...
			TextFieldValidator passValidator = new TextFieldValidator(
					txtPassword) {
			...
			TextFieldValidator passAgainValidator = new TextFieldValidator(
					txtPasswordAgain) {
			...
			txtPasswordAgain.getDocument().addDocumentListener(
					passAgainValidator);

		}
	}

	private static abstract class TextFieldValidator implements
